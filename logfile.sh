#!/bin/bash
LOG_FILE="/home/ec2-user/shell-practice/mycolorlog.log"
 #!/bin/bash 
...
Y='\033[33m'
BL='\033[0;34m'
echo -e " $R today date is $DATE $N" >>$LOG_FILE # -e is the one enables colore 
echo -e "$R sum is $sum $N"
echo -e "$NC sum is $sum $N" >>$LOG_FILE
...
echo -e "$BL practicing from bangalore today date us 19-10-2025$N" >>$LOG_FILE
